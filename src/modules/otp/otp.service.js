...
const list = async (query = {}) => {
  ...
  const [items, total] = await Promise.all([
    OTP.find(filter).skip(skip).limit(limit).sort({ createdAt: -1 }),
...
const getById = (id) => OTP.findById(id);
const updateById = (id, data) =>
  OTP.findByIdAndUpdate(id, data, { new: true, runValidators: true });
const deleteById = (id) => OTP.findByIdAndDelete(id);
